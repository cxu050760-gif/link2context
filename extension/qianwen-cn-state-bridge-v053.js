(() => {
  ...
  function showToast(message, error = false) {
    ...
    if (!toast) {
      ...
      Object.assign(toast.style, {
        ...
        maxWidth: '460px', padding: '10px 14px', borderRadius: '10px',
    ...
    showToast.timer = setTimeout(() => toast.remove(), error ? 7000 : 3500);
  }
  ...
  async function writeRealEditorState(editor, text, job) {
    assertActive(job);
    ...
    // This is the critical live-site gate. The previous generic DOM fallback could
    // paint text that Qianwen/QwenWork did not actually own. We only accept the
    // write if the site's own Send control becomes enabled from this text alone.
    const button = findSendButton(current);
    if (!button) return null;
    return { editor: current, button, signature };
  }
  ...
  function bodyTextWithoutComposer() {
    try {
      ...
      for (const item of body.querySelectorAll('[contenteditable="true"],[data-lexical-editor="true"],.ProseMirror,textarea,input[type="text"],input[type="search"],input[type="url"]')) item.remove();
      return normalize(body.innerText || body.textContent || '');
    } catch { return ''; }
  }

  async function submitVerified(editor, preferred, signature, job) {
    let current = currentComposer(editor);
    let button = findSendButton(current, preferred);
    const until = Date.now() + 6000;
    while (!button && Date.now() < until) {
      assertActive(job);
      await sleep(200, job);
      current = currentComposer(current || editor);
    ...
    while (Date.now() < verifyUntil) {
      ...
      const after = normalize(editorText(now));
  ...
  function attachmentScore(el) {
    ...
    if (/(image|photo|camera|图片|照片|相机)/i.test(text)) score -= 4;
  ...
  async function findFileInput(editor, file, job) {
    ...
    const scope = composerScope(editor);
    const control = [...scope.querySelectorAll('button,[role="button"],[role="menuitem"],[aria-label],[title]')]
      .filter(visible)
  ...
  function filenameHints(name) {
    ...
    return [...new Set([value, stem, stem.length >= 12 ? stem.slice(0, Math.min(24, stem.length)) : ''].filter((x) => x.length >= 8))];
  }
  ...
  async function attachBinary(result, editor, job) {
    ...
    while (Date.now() < deadline) {
      ...
      const shown = filenameHints(file.name).some((hint) => body.includes(hint.toLowerCase()));
      const button = findSendButton(current || editor);
      if (shown && button) return { editor: current || editor, button, fileName: file.name };
      await sleep(250, job);
    }
    return null;
  }
  ...
  async function start(editor, url, { preferredButton = null } = {}) {
    if (activeJob?.busy) return false;
    const job = { editor, url, busy: true, cancelled: false, autoSubmit: await autoSendEnabled() };
    activeJob = job;
    try {
      report('qianwen-cn-start', '千问中国站专用适配 / Chinese Qwen adapter', `Host: ${host}`);
      const result = await resolveUrl(url);
      assertActive(job);
      if (!result?.ok) throw Object.assign(new Error(result?.error || '链接读取失败'), { l2cCode: result?.errorCode || 'PIPELINE_ERROR' });
      ...
      if (originalBinary) {
        ...
        if (!proof) throw Object.assign(new Error('千问附件没有进入真实可发送状态。'), { l2cCode: 'QIANWEN_ATTACHMENT_STATE_UNCONFIRMED' });
        if (job.autoSubmit) {
          if (!(await submitVerified(proof.editor, proof.button || preferredButton, [], job))) {
            throw Object.assign(new Error('千问附件自动发送未确认。'), { l2cCode: 'SEND_UNCONFIRMED' });
          }
          report('sent', '附件已发送 / Attachment sent', proof.fileName, { state: 'success' });
        } else {
          report('ready-in-composer', '附件已准备 / Attachment ready', proof.fileName, { state: 'success' });
        }
        return true;
      }

      const text = resultText(result);
      if (text.length > MAX_INLINE_CHARS) throw Object.assign(new Error('内容过长，千问中国站暂不自动写入。'), { l2cCode: 'QIANWEN_CONTENT_TOO_LARGE' });
      const proof = await writeRealEditorState(currentComposer(editor) || editor, text, job);
      if (!proof) {
        throw Object.assign(new Error('千问没有确认这段内容进入真实编辑状态；已停止，避免出现删不掉但发不出去的假文本。'), { l2cCode: 'QIANWEN_EDITOR_STATE_UNCONFIRMED' });
      }

      report('qianwen-cn-state-confirmed', '千问真实编辑状态已确认 / Editor state confirmed', '内容本身已让千问发送控件变为可用。');
      if (job.autoSubmit) {
        if (!(await submitVerified(proof.editor, proof.button || preferredButton, proof.signature, job))) {
          throw Object.assign(new Error('千问自动发送未确认。'), { l2cCode: 'SEND_UNCONFIRMED' });
        }
        report('sent', '已发送 / Sent', '已取得独立页面证据。', { state: 'success' });
        showToast('Link2Context：千问内容已进入真实状态并发送。');
      } else {
        report('ready-in-composer', '内容已准备 / Ready', '现在应可正常修改、删除和发送。', { state: 'success' });
        showToast('Link2Context：千问内容已进入真实编辑状态。');
      }
      return true;
    } catch (error) {
      const code = String(error?.l2cCode || 'HANDOFF_ERROR');
      if (code === 'USER_CANCELLED' || job.cancelled) {
        report('cancelled', '已停止 / Stopped', '当前千问任务已停止。', { state: 'success', code: 'USER_CANCELLED' });
        return false;
      }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!editor) return;
    const url = singleUrl(editorText(editor));
    if (!url || sendScore(button, editor, composerScope(editor)) < 5) return;
    stopEvent(event);
