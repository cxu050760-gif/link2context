...
> 当前测试分支：**V0.5.1**。V0.5.1 新增“交付形式”和“发送行为”两个独立选项：Markdown 文档 / 长文本，以及手动确认（默认）/ 自动发送；同时修复网页 AI 已显示附件但因文件名截断而被误判为上传失败的问题，并继续加固千问的附件入口和富文本编辑器兼容。
...
V0.5.1 额外加入针对以下问题的攻击式回归：发送偏好 fail-safe、误自动发送、附件文件名截断、短名称误判、千问 `+ / 更多` 菜单、图片入口误选、禁用发送按钮强行启用、手动模式误报失败等。
